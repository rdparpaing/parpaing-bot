...
module.exports = async (id) => {
  var res = await supabase.from("srs").select("messages_count,lastmsg_ts,rating").eq("discord_id", id);
  if (res.data.length > 0) {
    ...
    res = await supabase
      .from("srs")
      .insert({ discord_id: id, rating: "0", lastmsg_ts: new Date().toISOString(), messages_count: 1});
    return 1;
  }
}